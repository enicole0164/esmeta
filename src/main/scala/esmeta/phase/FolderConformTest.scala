...
/** `folder-conform-test` phase */
case object FolderConformTest
  extends Phase[
    (
  ...
  def apply(
    input: (Input, Iterable[Script]),
    cmdConfig: CommandConfig,
    config: Config,
  ): Result =
    _config = config
    config.msgdir.foreach(cleanDir)
    cleanDir(s"$FOLDERCONFORMTEST_LOG_DIR/msg")

    val (etestMap, originals) = input

    // generate name->original map
    originals.foreach(s => originalMap += (s.name -> s.code))

    // load bug db
    val bugDB = loadBugDB(etestMap.keys)
    ...
    val result: Result = engineResult

    // dump result as JSON and tsv
    dumpJson(result.map(_.toString -> _), s"$FOLDERCONFORMTEST_LOG_DIR/fails.json")
    dumpJson(
      bugStat.map(_.toString -> _),
      s"$FOLDERCONFORMTEST_LOG_DIR/bug-stat.json",
    )
  ...
  private def doConformTest(
    ...
    if pass then None
      ...
      dumpMsg(s"$FOLDERCONFORMTEST_LOG_DIR/msg/$target")
  ...
  private def dumpSummary(
    ...
    val body: List[Vector[String]] = result
      ...
      .toList
    dumpRows(header :: body, s"$FOLDERCONFORMTEST_LOG_DIR/test-summary.tsv")
