...
$assert.sameValue($Object_getPrototypeOf(fulfiller), Object.prototype);
$assert.sameValue(Object.isExtensible(fulfiller), true);
$assert.notCallable(fulfiller);
$assert.notConstructable(fulfiller);
$assert.compareArray($Reflect_ownKeys(fulfiller), ['then'], fulfiller);
$assert.sameValue($Object_getPrototypeOf(fulfiller?.["then"]), Function.prototype);
$assert.sameValue(Object.isExtensible(fulfiller?.["then"]), true);
$assert.callable(fulfiller?.["then"]);
$assert.notConstructable(fulfiller?.["then"]);
$assert.compareArray($Reflect_ownKeys(fulfiller?.["then"]), ['length', 'name'], fulfiller?.["then"]);
$verifyProperty(fulfiller?.["then"], "name", {
  value: "then",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(fulfiller?.["then"], "length", {
  value: 1.0,
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(fulfiller, "then", {
  writable: true,
  enumerable: true,
  configurable: true,
});
