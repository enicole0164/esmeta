...
$assert.sameValue($Object_getPrototypeOf(thenable), Object.prototype);
$assert.sameValue(Object.isExtensible(thenable), true);
$assert.notCallable(thenable);
$assert.notConstructable(thenable);
$assert.compareArray($Reflect_ownKeys(thenable), ['then'], thenable);
$assert.sameValue($Object_getPrototypeOf(thenable?.["then"]), Function.prototype);
$assert.sameValue(Object.isExtensible(thenable?.["then"]), true);
$assert.callable(thenable?.["then"]);
$assert.notConstructable(thenable?.["then"]);
$assert.compareArray($Reflect_ownKeys(thenable?.["then"]), ['length', 'name'], thenable?.["then"]);
$verifyProperty(thenable?.["then"], "name", {
  value: "then",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(thenable?.["then"], "length", {
  value: 2.0,
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(thenable, "then", {
  writable: true,
  enumerable: true,
  configurable: true,
});
