...
$assert.sameValue($Object_getPrototypeOf(globalThis["resolver"]), Object.prototype);
$assert.sameValue(Object.isExtensible(globalThis["resolver"]), true);
$assert.notCallable(globalThis["resolver"]);
$assert.notConstructable(globalThis["resolver"]);
$assert.compareArray($Reflect_ownKeys(globalThis["resolver"]), ['then'], globalThis["resolver"]);
$assert.sameValue($Object_getPrototypeOf(globalThis["resolver"]?.["then"]), Function.prototype);
$assert.sameValue(Object.isExtensible(globalThis["resolver"]?.["then"]), true);
$assert.callable(globalThis["resolver"]?.["then"]);
$assert.notConstructable(globalThis["resolver"]?.["then"]);
$assert.compareArray($Reflect_ownKeys(globalThis["resolver"]?.["then"]), ['length', 'name'], globalThis["resolver"]?.["then"]);
$verifyProperty(globalThis["resolver"]?.["then"], "name", {
  value: "then",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(globalThis["resolver"]?.["then"], "length", {
  value: 1.0,
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(globalThis["resolver"], "then", {
  writable: true,
  enumerable: true,
  configurable: true,
});
