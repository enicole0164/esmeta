...
$assert.sameValue($Object_getPrototypeOf(boundThen), Function.prototype);
$assert.sameValue(Object.isExtensible(boundThen), true);
$assert.callable(boundThen);
$assert.notConstructable(boundThen);
$assert.compareArray($Reflect_ownKeys(boundThen), ['length', 'name'], boundThen);
$verifyProperty(boundThen, "name", {
  value: "bound then",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(boundThen, "length", {
  value: 2.0,
  writable: false,
  enumerable: false,
  configurable: true,
