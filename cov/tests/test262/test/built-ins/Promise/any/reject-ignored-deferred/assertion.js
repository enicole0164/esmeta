...
$assert.sameValue(callCount, 4.0);
...
$assert.sameValue($Object_getPrototypeOf(rejector), Object.prototype);
$assert.sameValue(Object.isExtensible(rejector), true);
$assert.notCallable(rejector);
$assert.notConstructable(rejector);
$assert.compareArray($Reflect_ownKeys(rejector), ['then'], rejector);
$assert.sameValue($Object_getPrototypeOf(rejector?.["then"]), Function.prototype);
$assert.sameValue(Object.isExtensible(rejector?.["then"]), true);
$assert.callable(rejector?.["then"]);
$assert.notConstructable(rejector?.["then"]);
$assert.compareArray($Reflect_ownKeys(rejector?.["then"]), ['length', 'name'], rejector?.["then"]);
$verifyProperty(rejector?.["then"], "name", {
  value: "then",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(rejector?.["then"], "length", {
  value: 2.0,
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(rejector, "then", {
  writable: true,
  enumerable: true,
  configurable: true,
});
