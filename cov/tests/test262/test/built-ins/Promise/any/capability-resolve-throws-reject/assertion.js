...
$assert.sameValue(callCount, 6.0);
$assert.sameValue($Object_getPrototypeOf(thrown), globalThis["Test262Error"]?.["prototype"]);
$assert.sameValue(Object.isExtensible(thrown), true);
$assert.notCallable(thrown);
$assert.notConstructable(thrown);
$assert.compareArray($Reflect_ownKeys(thrown), ['message'], thrown);
$verifyProperty(thrown, "message", {
  value: "",
  writable: true,
  enumerable: true,
  configurable: true,
