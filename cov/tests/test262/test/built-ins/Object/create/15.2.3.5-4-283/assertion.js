...
$assert.sameValue($Object_getPrototypeOf(globalThis["numObj"]), Number.prototype);
$assert.sameValue(Object.isExtensible(globalThis["numObj"]), true);
$assert.notCallable(globalThis["numObj"]);
$assert.notConstructable(globalThis["numObj"]);
$assert.compareArray($Reflect_ownKeys(globalThis["numObj"]), ['set'], globalThis["numObj"]);
$assert.sameValue(globalThis["numObj"]?.["set"], Object.getOwnPropertyDescriptor(globalThis["newObj"], "prop")?.set);
$verifyProperty(globalThis["numObj"], "set", {
  writable: true,
  enumerable: true,
  configurable: true,
});
