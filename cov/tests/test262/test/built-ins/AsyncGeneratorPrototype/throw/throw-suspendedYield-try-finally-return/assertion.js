...
$assert.sameValue($Object_getPrototypeOf(Err), Error);
$assert.sameValue(Object.isExtensible(Err), true);
$assert.callable(Err);
$assert.constructable(Err);
$assert.compareArray($Reflect_ownKeys(Err), ['length', 'name', 'prototype'], Err);
$assert.sameValue($Object_getPrototypeOf(Err?.["prototype"]), Error.prototype);
$assert.sameValue(Object.isExtensible(Err?.["prototype"]), true);
$assert.notCallable(Err?.["prototype"]);
$assert.notConstructable(Err?.["prototype"]);
$assert.compareArray($Reflect_ownKeys(Err?.["prototype"]), ['constructor'], Err?.["prototype"]);
$assert.sameValue(Err?.["prototype"]?.["constructor"], Err);
$verifyProperty(Err?.["prototype"], "constructor", {
  writable: true,
  enumerable: false,
  configurable: true,
});
$verifyProperty(Err, "prototype", {
  writable: false,
  enumerable: false,
  configurable: false,
});
$verifyProperty(Err, "name", {
  value: "Err",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(Err, "length", {
  value: 0.0,
  writable: false,
  enumerable: false,
  configurable: true,
