...
$assert.sameValue(_, undefined);
$assert.sameValue($Object_getPrototypeOf(args), Object.prototype);
$assert.sameValue(Object.isExtensible(args), true);
$assert.notCallable(args);
$assert.notConstructable(args);
$assert.compareArray($Reflect_ownKeys(args), ['length', 'callee', Symbol.iterator], args);
$assert.sameValue(args?.[Symbol.iterator], Array.prototype.values);
$verifyProperty(args, Symbol.iterator, {
  writable: true,
  enumerable: false,
  configurable: true,
});
$assert.sameValue(Object.getOwnPropertyDescriptor(args, "callee")?.get, ThrowTypeError);
$assert.sameValue(Object.getOwnPropertyDescriptor(args, "callee")?.set, ThrowTypeError);
$verifyProperty(args, "callee", {
  enumerable: false,
  configurable: false,
});
$verifyProperty(args, "length", {
  value: 0.0,
  writable: true,
  enumerable: false,
  configurable: true,
});
$assert.sameValue(iterCount, 1.0);
...
$assert.sameValue($Object_getPrototypeOf(promise), Promise.prototype);
$assert.sameValue(Object.isExtensible(promise), true);
$assert.notCallable(promise);
$assert.notConstructable(promise);
$assert.compareArray($Reflect_ownKeys(promise), [], promise);
$assert.sameValue(returnCount, 1.0);
$assert.sameValue(thisValue, iterator);
