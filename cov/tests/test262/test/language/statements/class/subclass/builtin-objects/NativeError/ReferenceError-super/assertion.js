...
$assert.sameValue($Object_getPrototypeOf(CustomError), ReferenceError);
$assert.sameValue(Object.isExtensible(CustomError), true);
$assert.callable(CustomError);
$assert.constructable(CustomError);
$assert.compareArray($Reflect_ownKeys(CustomError), ['length', 'name', 'prototype'], CustomError);
$assert.sameValue($Object_getPrototypeOf(CustomError?.["prototype"]), ReferenceError.prototype);
$assert.sameValue(Object.isExtensible(CustomError?.["prototype"]), true);
$assert.notCallable(CustomError?.["prototype"]);
$assert.notConstructable(CustomError?.["prototype"]);
$assert.compareArray($Reflect_ownKeys(CustomError?.["prototype"]), ['constructor'], CustomError?.["prototype"]);
$assert.sameValue(CustomError?.["prototype"]?.["constructor"], CustomError);
$verifyProperty(CustomError?.["prototype"], "constructor", {
  writable: true,
  enumerable: false,
  configurable: true,
});
$verifyProperty(CustomError, "prototype", {
  writable: false,
  enumerable: false,
  configurable: false,
});
$verifyProperty(CustomError, "name", {
  value: "CustomError",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(CustomError, "length", {
  value: 0.0,
  writable: false,
  enumerable: false,
  configurable: true,
