...
$assert.sameValue($Object_getPrototypeOf(Derived), Base);
$assert.sameValue(Object.isExtensible(Derived), true);
$assert.callable(Derived);
$assert.constructable(Derived);
$assert.compareArray($Reflect_ownKeys(Derived), ['length', 'name', 'prototype'], Derived);
$assert.sameValue($Object_getPrototypeOf(Derived?.["prototype"]), Base?.["prototype"]);
$assert.sameValue(Object.isExtensible(Derived?.["prototype"]), true);
$assert.notCallable(Derived?.["prototype"]);
$assert.notConstructable(Derived?.["prototype"]);
$assert.compareArray($Reflect_ownKeys(Derived?.["prototype"]), ['constructor'], Derived?.["prototype"]);
$assert.sameValue(Derived?.["prototype"]?.["constructor"], Derived);
$verifyProperty(Derived?.["prototype"], "constructor", {
  writable: true,
  enumerable: false,
  configurable: true,
});
$verifyProperty(Derived, "prototype", {
  writable: false,
  enumerable: false,
  configurable: false,
});
$verifyProperty(Derived, "name", {
  value: "Derived",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(Derived, "length", {
  value: 0.0,
  writable: false,
  enumerable: false,
  configurable: true,
});
$assert.sameValue($Object_getPrototypeOf(instance), Derived?.["prototype"]);
$assert.sameValue(Object.isExtensible(instance), true);
$assert.notCallable(instance);
$assert.notConstructable(instance);
$assert.compareArray($Reflect_ownKeys(instance), ['value'], instance);
$verifyProperty(instance, "value", {
  value: 5.0,
  writable: true,
  enumerable: true,
  configurable: true,
