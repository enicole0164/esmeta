...
$assert.compareArray($Reflect_ownKeys(C), ['length', 'name', 'prototype', 'getWithEval'], C);
...
$assert.sameValue($Object_getPrototypeOf(C?.["getWithEval"]), Function.prototype);
$assert.sameValue(Object.isExtensible(C?.["getWithEval"]), true);
$assert.callable(C?.["getWithEval"]);
$assert.notConstructable(C?.["getWithEval"]);
$assert.compareArray($Reflect_ownKeys(C?.["getWithEval"]), ['length', 'name'], C?.["getWithEval"]);
$verifyProperty(C?.["getWithEval"], "name", {
  value: "getWithEval",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(C?.["getWithEval"], "length", {
  value: 0.0,
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(C, "getWithEval", {
  writable: true,
  enumerable: false,
  configurable: true,
});
