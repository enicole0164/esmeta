...
$assert.compareArray($Reflect_ownKeys(C), ['length', 'name', 'prototype', 'f'], C);
$assert.sameValue($Object_getPrototypeOf(C?.["f"]), Function.prototype);
$assert.sameValue(Object.isExtensible(C?.["f"]), true);
$assert.callable(C?.["f"]);
$assert.notConstructable(C?.["f"]);
$assert.compareArray($Reflect_ownKeys(C?.["f"]), ['length', 'name'], C?.["f"]);
$verifyProperty(C?.["f"], "name", {
  value: "f",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(C?.["f"], "length", {
  value: 0.0,
  writable: false,
  enumerable: false,
  configurable: true,
