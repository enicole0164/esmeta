...
$assert.compareArray($Reflect_ownKeys(C?.["B"]), ['length', 'name', 'prototype', 'access'], C?.["B"]);
$assert.sameValue($Object_getPrototypeOf(C?.["B"]?.["access"]), Function.prototype);
$assert.sameValue(Object.isExtensible(C?.["B"]?.["access"]), true);
$assert.callable(C?.["B"]?.["access"]);
$assert.notConstructable(C?.["B"]?.["access"]);
$assert.compareArray($Reflect_ownKeys(C?.["B"]?.["access"]), ['length', 'name'], C?.["B"]?.["access"]);
$verifyProperty(C?.["B"]?.["access"], "name", {
  value: "access",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(C?.["B"]?.["access"], "length", {
  value: 1.0,
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(C?.["B"], "access", {
  writable: true,
  enumerable: false,
  configurable: true,
});
