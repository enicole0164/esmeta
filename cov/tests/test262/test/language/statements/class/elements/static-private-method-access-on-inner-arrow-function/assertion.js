...
$assert.compareArray($Reflect_ownKeys(C), ['length', 'name', 'prototype', 'g'], C);
$assert.sameValue($Object_getPrototypeOf(C?.["g"]), Function.prototype);
$assert.sameValue(Object.isExtensible(C?.["g"]), true);
$assert.callable(C?.["g"]);
$assert.notConstructable(C?.["g"]);
$assert.compareArray($Reflect_ownKeys(C?.["g"]), ['length', 'name'], C?.["g"]);
$verifyProperty(C?.["g"], "name", {
  value: "g",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(C?.["g"], "length", {
  value: 0.0,
  writable: false,
  enumerable: false,
  configurable: true,
