...
$assert.compareArray($Reflect_ownKeys(C), ['length', 'name', 'prototype', 'staticFunc', 'staticProp'], C);
$verifyProperty(C, "staticProp", {
...
$assert.sameValue($Object_getPrototypeOf(C?.["staticFunc"]), Function.prototype);
$assert.sameValue(Object.isExtensible(C?.["staticFunc"]), true);
$assert.callable(C?.["staticFunc"]);
$assert.notConstructable(C?.["staticFunc"]);
$assert.compareArray($Reflect_ownKeys(C?.["staticFunc"]), ['length', 'name'], C?.["staticFunc"]);
$verifyProperty(C?.["staticFunc"], "name", {
  value: "staticFunc",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(C?.["staticFunc"], "length", {
  value: 0.0,
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(C, "staticFunc", {
  writable: true,
  enumerable: true,
  configurable: true,
});
