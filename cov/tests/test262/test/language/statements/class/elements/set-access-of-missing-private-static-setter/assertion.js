...
$assert.compareArray($Reflect_ownKeys(C), ['length', 'name', 'prototype', 'setAccess'], C);
$assert.sameValue($Object_getPrototypeOf(C?.["setAccess"]), Function.prototype);
$assert.sameValue(Object.isExtensible(C?.["setAccess"]), true);
$assert.callable(C?.["setAccess"]);
$assert.notConstructable(C?.["setAccess"]);
$assert.compareArray($Reflect_ownKeys(C?.["setAccess"]), ['length', 'name'], C?.["setAccess"]);
$verifyProperty(C?.["setAccess"], "name", {
  value: "setAccess",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(C?.["setAccess"], "length", {
  value: 0.0,
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(C, "setAccess", {
  writable: true,
  enumerable: false,
  configurable: true,
});
