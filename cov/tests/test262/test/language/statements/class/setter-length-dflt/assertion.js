...
$assert.sameValue($Object_getPrototypeOf(globalThis["set"]), Function.prototype);
$assert.sameValue(Object.isExtensible(globalThis["set"]), true);
$assert.callable(globalThis["set"]);
$assert.notConstructable(globalThis["set"]);
$assert.compareArray($Reflect_ownKeys(globalThis["set"]), ['name'], globalThis["set"]);
$verifyProperty(globalThis["set"], "name", {
  value: "set m",
  writable: false,
  enumerable: false,
  configurable: true,
...
$assert.sameValue(Object.getOwnPropertyDescriptor(C?.["prototype"], "m")?.set, globalThis["set"]);
