...
$assert.compareArray($Reflect_ownKeys(actual), ['0', '1', '2', 'length'], actual);
...
$assert.compareArray($Reflect_ownKeys(expected), ['0', '1', '2', 'length'], expected);
...
$assert.sameValue($Object_getPrototypeOf(patched), Promise.prototype);
$assert.sameValue(Object.isExtensible(patched), true);
$assert.notCallable(patched);
$assert.notConstructable(patched);
$assert.compareArray($Reflect_ownKeys(patched), ['then'], patched);
$assert.sameValue($Object_getPrototypeOf(patched?.["then"]), Function.prototype);
$assert.sameValue(Object.isExtensible(patched?.["then"]), true);
$assert.callable(patched?.["then"]);
$assert.constructable(patched?.["then"]);
$assert.compareArray($Reflect_ownKeys(patched?.["then"]), ['length', 'name', 'prototype'], patched?.["then"]);
$assert.sameValue($Object_getPrototypeOf(patched?.["then"]?.["prototype"]), Object.prototype);
$assert.sameValue(Object.isExtensible(patched?.["then"]?.["prototype"]), true);
$assert.notCallable(patched?.["then"]?.["prototype"]);
$assert.notConstructable(patched?.["then"]?.["prototype"]);
$assert.compareArray($Reflect_ownKeys(patched?.["then"]?.["prototype"]), ['constructor'], patched?.["then"]?.["prototype"]);
$assert.sameValue(patched?.["then"]?.["prototype"]?.["constructor"], patched?.["then"]);
$verifyProperty(patched?.["then"]?.["prototype"], "constructor", {
  writable: true,
  enumerable: false,
  configurable: true,
});
$verifyProperty(patched?.["then"], "prototype", {
  writable: true,
  enumerable: false,
  configurable: false,
});
$verifyProperty(patched?.["then"], "name", {
  value: "",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(patched?.["then"], "length", {
  value: 0.0,
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(patched, "then", {
  writable: true,
  enumerable: true,
  configurable: true,
});
$assert.sameValue(thenCallCount, 0.0);
$assert.sameValue(value, 42.0);
