...
$assert.sameValue($Object_getPrototypeOf(b), Object.prototype);
$assert.sameValue(Object.isExtensible(b), true);
$assert.notCallable(b);
$assert.notConstructable(b);
$assert.compareArray($Reflect_ownKeys(b), ['e'], b);
$verifyProperty(b, "e", {
  value: 0.0,
  writable: true,
  enumerable: true,
...
$assert.sameValue($Object_getPrototypeOf(d), Array.prototype);
$assert.sameValue(Object.isExtensible(d), true);
$assert.notCallable(d);
$assert.notConstructable(d);
$assert.compareArray($Reflect_ownKeys(d), ['0', 'length'], d);
