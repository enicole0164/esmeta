...
$assert.sameValue($Object_getPrototypeOf(globalThis["C"]?.["prototype"]?.["method"]), AsyncFunction.prototype);
...
$assert.sameValue($Object_getPrototypeOf(asyncFn), AsyncFunction.prototype);
$assert.sameValue(Object.isExtensible(asyncFn), true);
$assert.callable(asyncFn);
$assert.notConstructable(asyncFn);
$assert.compareArray($Reflect_ownKeys(asyncFn), ['length', 'name'], asyncFn);
$verifyProperty(asyncFn, "name", {
  value: "bound method",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(asyncFn, "length", {
  value: 1.0,
  writable: false,
  enumerable: false,
  configurable: true,
