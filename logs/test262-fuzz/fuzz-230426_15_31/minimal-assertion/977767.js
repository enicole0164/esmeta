...
$assert.compareArray($Reflect_ownKeys(C), ['length', 'name', 'prototype', 'getAccess'], C);
$assert.sameValue($Object_getPrototypeOf(C?.["getAccess"]), Function.prototype);
$assert.sameValue(Object.isExtensible(C?.["getAccess"]), true);
$assert.callable(C?.["getAccess"]);
$assert.notConstructable(C?.["getAccess"]);
$assert.compareArray($Reflect_ownKeys(C?.["getAccess"]), ['length', 'name'], C?.["getAccess"]);
$verifyProperty(C?.["getAccess"], "name", {
  value: "getAccess",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(C?.["getAccess"], "length", {
  value: 0.0,
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(C, "getAccess", {
  writable: true,
  enumerable: false,
  configurable: true,
});
