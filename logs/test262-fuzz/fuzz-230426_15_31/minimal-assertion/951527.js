...
$assert.compareArray($Reflect_ownKeys(globalThis["assert"]), ['length', 'name', 'prototype', '_isSameValue', 'sameValue'], globalThis["assert"]);
...
$assert.sameValue(bigint, 9007199254740991n);
$assert.sameValue($Object_getPrototypeOf(m), Map.prototype);
$assert.sameValue(Object.isExtensible(m), true);
$assert.notCallable(m);
$assert.notConstructable(m);
$assert.compareArray($Reflect_ownKeys(m), [], m);
$assert.sameValue(number, 9.007199254740991E15);
