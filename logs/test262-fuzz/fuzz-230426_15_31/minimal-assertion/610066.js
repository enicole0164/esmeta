...
$assert.sameValue($Object_getPrototypeOf(x?.["prototype"]?.[""]), Function.prototype);
$assert.sameValue(Object.isExtensible(x?.["prototype"]?.[""]), true);
$assert.callable(x?.["prototype"]?.[""]);
$assert.notConstructable(x?.["prototype"]?.[""]);
$assert.compareArray($Reflect_ownKeys(x?.["prototype"]?.[""]), ['length', 'name'], x?.["prototype"]?.[""]);
$verifyProperty(x?.["prototype"]?.[""], "name", {
  value: "",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(x?.["prototype"]?.[""], "length", {
  value: 0.0,
  writable: false,
  enumerable: false,
  configurable: true,
