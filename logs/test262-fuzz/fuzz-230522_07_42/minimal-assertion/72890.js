...
$assert.sameValue($Object_getPrototypeOf(resolver), Object.prototype);
$assert.sameValue(Object.isExtensible(resolver), true);
$assert.notCallable(resolver);
$assert.notConstructable(resolver);
$assert.compareArray($Reflect_ownKeys(resolver), ['then'], resolver);
$assert.sameValue($Object_getPrototypeOf(resolver?.["then"]), Function.prototype);
$assert.sameValue(Object.isExtensible(resolver?.["then"]), true);
$assert.callable(resolver?.["then"]);
$assert.notConstructable(resolver?.["then"]);
$assert.compareArray($Reflect_ownKeys(resolver?.["then"]), ['length', 'name'], resolver?.["then"]);
$verifyProperty(resolver?.["then"], "name", {
  value: "then",
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(resolver?.["then"], "length", {
  value: 1.0,
  writable: false,
  enumerable: false,
  configurable: true,
});
$verifyProperty(resolver, "then", {
  writable: true,
  enumerable: true,
  configurable: true,
});
